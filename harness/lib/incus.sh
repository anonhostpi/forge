...
# vm_create <name> <image> <userdata-file> — user-data set BEFORE first start so cloud-init runs it (see forge#3 review finding 1)
vm_create() {
...
# re-apply cloud-init on an already-provisioned box — the dirty/upgrade replay path (see forge#3 review finding 3)
